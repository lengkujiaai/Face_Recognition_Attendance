...
class Record:
    def __init__(self):
        ...
        self.csv_file = str(datetime.now()).split(' ')[0]+".csv"
        shutil.copy("empty.csv", self.csv_file)
    ...
    def time_about(self):
        hour_str = str(datetime.now()).split(' ')[1].split(":")[0]
        hour_int = int(hour_str)
        if hour_int > 5 and hour_int < 21:
            return True
        else:
            self.csv_file = str(datetime.now()).split(' ')[0]+".csv"
            shutil.copy("empty.csv", self.csv_file)
        ...
        while True:
            ...
            if len(facesCurFrame) != 0:
                ...
                if self.className[matchIndex]:
                    ...
                    cv2.putText(frame, name, (50, 50), cv2.FONT_HERSHEY_DUPLEX, 1.0, (0, 255, 0), 1)
                    cv2.putText(frame, str(faceDis[matchIndex]), (50, 80), cv2.FONT_HERSHEY_DUPLEX, 1.0, (0, 255, 0), 1)
                else:
                    name = 'Unknown'
                    self.markAttendance(name)
